...
async def get_order_user(db: AsyncSession,
                         user_id: int) -> List[Order] | None:
    db_item = await db.execute(select(Order).
...
async def lock_active_cart(db: AsyncSession,
                           order_id: int) -> Cart | None:
...
async def confirmed_orders(db: AsyncSession,
                           order_id: int) -> Order:
    item = await db.execute(select(Order).
...
async def create_payments(db: AsyncSession,
                          order_id: int) -> Payment | None:
...
async def check_payment(db: AsyncSession, order_id: int) -> Payment | None:
    payment = await db.execute(select(Payment).join(Order).
                               filter(Order.id == order_id))
